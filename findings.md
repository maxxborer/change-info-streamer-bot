...
- `YouTubeUserInfo.UserId` is the connected channel ID. API v3 exposes it only as a Studio URL input: an active broadcast uses its video Studio URL; every non-live state uses the channel-level livestreaming dashboard URL, so a completed monitored broadcast is not opened by mistake.
