...
«Шаблоны» — первый рабочий блок. В каждом шаблоне допускается не более одного `%subtitle%`.
...
`npm run build` пересобирает нативную Import-строку Streamer.bot, проверяет TypeScript и создаёт четыре самостоятельных HTML. `dist/` — локальная папка сборки и намеренно не хранится в Git.
